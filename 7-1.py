...
for x in a:
    if len(x)>1:
        ...
        for y in x[1].split(', '):
            tree[y] = name
    w[x[0].split()[0]] = int(x[0].split()[1][1:-1])

root = [x[0].split()[0] for x in a if x[0].split()[0] not in tree][0]

def get_branches(node):
    return [x for x in tree if tree[x] == node]
...
def get_total_weight(node):
    ...
    if len(b) == 0:
        ...
        return w[node] + sum([get_total_weight(x) for x in b])
...
def validate(node, n):
    b = get_branches(node)
    bw = [get_total_weight(x) for x in b]
    if all_equal(bw):
        validate(tree[node], n+1)
    else:
        print (node, n, b, bw)

def get_leaves(a):
    return [x[0].split()[0] for x in a if len(x) == 1]

leaves = get_leaves(a)

br = get_branches('jdxfsa')

print [(x, get_total_weight(x)) for x in br]
